...
| `nis_macro_pa_points.mac` | 5 | Step 4 – PA batch points | Builds an N-point ND multipoint (`ND_AppendMultipointPoint`) from the first `count` `af_trigger`s, for the job's "Import Point Set from ND." | `af_trigger_01..N` + `pa_trigger.ini` → ND multipoint |
